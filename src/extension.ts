...
export function activate(context: vscode.ExtensionContext) {
    ...
    const chatView = vscode.window.registerWebviewViewProvider('neuroforge.chatView', chatViewProvider);
    ...
    let analyzeDependencies = vscode.commands.registerCommand('neuroforge.analyzeDependencies', () => {
    ...
    let openSettings = vscode.commands.registerCommand('neuroforge.openSettings', () => {
        chatViewProvider.toggleSettings();
    });
    ...
    let showMenu = vscode.commands.registerCommand('neuroforge.showMenu', () => {
        vscode.window.showQuickPick([
            ...
            if (selection) {
                vscode.commands.executeCommand(`neuroforge.${selection.toLowerCase().replace(/\s+/g, '')}`);
            }
        });
    });
